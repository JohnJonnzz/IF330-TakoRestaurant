    ...
    if (isset($_FILES['image'])) {
        ...
        if (in_array($image_extension, $allowed_extensions)) {
            ...
            if (move_uploaded_file($image_tmp_name, $target_path)) {
                $sql = "INSERT INTO product_list (category_id, category, name, description, price, img_path)
                        VALUES (?, ?, ?, ?, ?, ?)";
                $stmt = $db->prepare($sql);
                $stmt->execute([$category_id, $category, $product_name, $description, $price, $target_path]);
